...
func (h *AppHandlers) GetTripItineraryHandler(w http.ResponseWriter, r *http.Request) {
	...
	if len(itineraryDays) == 0 {
		...
		json.NewEncoder(w).Encode(map[string]string{"message": "Itinerary days not found for this trip."})
		return
	}
...
func (h *AppHandlers) GetActivitiesHandler(w http.ResponseWriter, r *http.Request) {
	...
	if len(activities) == 0 {
		w.WriteHeader(http.StatusNotFound)
		json.NewEncoder(w).Encode(map[string]string{"message": "No activities found for this itinerary day."})
		return
	}
